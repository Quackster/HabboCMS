...
require './inc/class.recaptchalib.php';
...
switch ($_GET['stap']) {
    case '1':
        ...
        if (isset($_SESSION['jjp']['register'][1])) {
            header('Location: ' . WWW . '/quickregister/email_password'); // /register.php?stap=3
        }
    ...
    case '2':
        ...
        if (!is_numeric($bday_day) || !is_numeric($bday_month) || !is_numeric($bday_year) || $bday_day <= 0 || $bday_day > 31 || $bday_month <= 0 || $bday_month > 12 || $bday_year < 1900 || $bday_year > 2014) {
            $errors = 'Bitte gib ein g&uuml;ltiges Geburtsdatum ein!'; /* HABBO_TRANSLATION_ERROR */
        } elseif (!empty($gender)) {
    ...
    case '5':
        ...
        if (!isset($_SESSION['jjp']['register'][1])) {
            ...
            if (!isset($_SESSION['jjp']['register'][2])) {
    ...
    case '6':
        ...
        $resp = recaptcha_check_answer('6Le-aQoAAAAAAKaqhlUT0lAQbjqokPqmj0F1uvQm', $_SERVER["REMOTE_ADDR"],
        ...
        if (!$resp->is_valid) {
            ...
            $users->add($_SESSION['jjp']['register'][2]['name'], $core->Hash($_SESSION['jjp']['register'][2]['pass']),
                $_SESSION['jjp']['register'][2]['email'], 1, $look, $gender);
