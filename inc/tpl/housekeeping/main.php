					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover">
							...
							<tbody>
								...
								$usersget = mysql_query("SELECT * FROM users");			$users = mysql_num_rows($usersget);		if($users <= 0) { $users = 0; }
								$roomsget = mysql_query("SELECT * FROM rooms");			$rooms = mysql_num_rows($roomsget);		if($rooms <= 0) { $rooms = 0; }
								$furniget = mysql_query("SELECT * FROM furniture");		$furni = mysql_num_rows($furniget);		if($furni <= 0) { $furni = 0; }
								$newsget  = mysql_query("SELECT * FROM site_news");		$newsg = mysql_num_rows($newsget);		if($newsg <= 0) { $newsg = 0; }
								$bansget  = mysql_query("SELECT * FROM users_bans");	$bans  = mysql_num_rows($bansget);		if($bans <= 0) { $bans = 0; }
								$tagsget  = mysql_query("SELECT * FROM user_tags"); 	$tags  = mysql_num_rows($tagsget);		if($tags <= 0) { $tags = 0; }
								$badgeget = mysql_query("SELECT * FROM user_badges");	$badge = mysql_num_rows($badgeget);		if($badge <= 0) { $badge = 0; }
								
								?>
								<style type="text/css">
									.no-link, .no-link:hover, .no-link:active { text-decoration: none; color: black; }
								</style>
								<tr>
